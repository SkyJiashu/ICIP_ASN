...
def make_dataset(dir, targetroot, extensions):
    images = []
    print(dir)
    for image in gb.glob(dir+"*.png"):
        
        name = image[image.rindex("/")+1:]
        idnum = name[:name.index("_")]
        tempdir = image[:image.rindex("/")]
        expression = name[:name.rindex("_")]
        expression = expression[:expression.rindex("_")]
        expression = expression[:expression.rindex("_")]
        expression = expression[:expression.rindex("_")]
        expression = expression[expression.index("_")+1:]
        expression = expression[expression.index("_")+1:]
        
        if (expression == "02"):
            continue
            
        trainKname = "LM_"+name + ".npz"

        dirK = tempdir+"K/"

        tempdir = targetroot[:targetroot.rindex("/")]
        targetK = tempdir+"K/"

        imagename = name
        num = name

        imagehalf = imagename[:imagename.rindex("_")]
        imagehalf = imagehalf[:imagehalf.rindex("_")]
        imagehalf = imagehalf[:imagehalf.rindex("_")]
        imagehalf = imagehalf[:imagehalf.rindex("_")]

        num = num[num.index("_")+1:]

        num = num[num.index("_")+1:]

        num = num[num.index("_")+1:]
        imagenamerest = num[num.index("_")+1:]

        num = num[:num.index("_")] # Get Degree

        imagename = imagehalf + "_051_" + imagenamerest
        testK = "LM_"+imagename + ".npz"

        targetimage = targetroot+imagename
        num = [int(num)]

        idnum = [int(idnum) - 1]

        trainimageK = dirK+trainKname
        targetimageK = targetK+testK


        exists = os.path.isfile(str(targetimage))

        if exists:
            exists = os.path.isfile(str(trainimageK))

            if exists:
                exists = os.path.isfile(str(targetimageK))

                if exists:
                    item = (image, str(targetimage) , str(trainimageK), str(targetimageK), num, idnum)
                    images.append(item)
...
class DatasetFolder(data.Dataset):
    ...
    def __init__(self, root, targetroot, loader, extensions, transform=None, R64_transform = None, R32_transform =None,R16_transform =None, Local_transform = None):

        print("root")
        samples = make_dataset(root, targetroot, extensions)
        if len(samples) == 0:
        ...
        self.R64_transform = R64_transform
        self.R32_transform = R32_transform
...
class ImageFolder(DatasetFolder):
    ...
    """
    def __init__(self, root, targetroot, transform=None, R64_transform=None, R32_transform= None,R16_transform= None, Local_transform = None):
        super(ImageFolder, self).__init__(root,targetroot, loader = default_loader, extensions = ['.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif'],
                                          transform=transform,
                                          R64_transform=R64_transform, R32_transform= R32_transform,R16_transform= R16_transform, Local_transform =Local_transform)
